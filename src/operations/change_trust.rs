        ...
        match self {
            ...
            Self::Credit(ref credit) => match credit {
                CreditAsset::AlphaNum4 { code, issuer } => {
                    ...
                    let asset_code = xdr::AssetCode4::try_from(code.as_bytes()).map_err(|_| Error::InvalidAssetCode)?;
                    let issuer = issuer.to_xdr_account_id()?;
                    let asset_alphanum4 = xdr::AlphaNum4 { asset_code, issuer };
                    Ok(xdr::ChangeTrustAsset::CreditAlphanum4(
                        asset_alphanum4,
                    ))
                }
                CreditAsset::AlphaNum12 { code, issuer } => {
                    let asset_code = xdr::AssetCode12::try_from(code.as_bytes()).map_err(|_| Error::InvalidAssetCode)?;
                    let issuer = issuer.to_xdr_account_id()?;
                    let asset_alphanum12 = xdr::AlphaNum12 { asset_code, issuer };
                    Ok(xdr::ChangeTrustAsset::CreditAlphanum12(
                        asset_alphanum12,
                    ))
                }
            },
        ...
        if let Some(limit) = &self.limit {
            if limit.to_i64() < 0 {
                return Err(Error::InvalidOperation(
                    "change trust limit must not be negative".to_string(),
                ));
            }
        }
