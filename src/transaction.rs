...
impl Transaction {
    ...
    pub fn to_xdr_transaction_signature_payload(
        ...
        let network_id = network.network_id().try_into().map_err(|_| Error::XdrError)?;
        let inner = self.to_xdr()?;
        let tagged_transaction =
            xdr::TransactionSignaturePayloadTaggedTransaction::Tx(inner);
...
impl TransactionEnvelope {
    ...
    pub fn from_xdr(x: &xdr::TransactionEnvelope) -> Result<TransactionEnvelope> {
        match x {
            ...
            xdr::TransactionEnvelope::TxV0(_) => todo!(),
...
fn signatures_to_xdr(signatures: &[DecoratedSignature]) -> Result<xdr::VecM<xdr::DecoratedSignature, 20>> {
